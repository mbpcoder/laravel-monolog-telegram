...
class TelegramFormatter implements FormatterInterface
{
	...
	private function getMessageForException($exception)
	{
		...
		$message = $severity . ' <b>Time: </b> ' . date('Y-m-d H:i:s') . PHP_EOL
			...
			. '<b>Code:</b> ' . $exception->getCode() . PHP_EOL
		...
		try {
			...
			if (Str::contains($exception->getMessage(), 'Telegram') && isset($exception->getTrace()[1]['args'][1]['chat_id'])) {
		...
		if (auth()->check()) {
			$message .= PHP_EOL . '<b>User:</b> ' . auth()->user()->id . ' / <b>Name:</b> ' . auth()->user()->fullName;
		}

		if (!empty($request->headers->get('referer'))) {
			$message .= PHP_EOL
				. '<b>Referer:</b> ' . $request->headers->get('referer');
		}
		...
		$message .= PHP_EOL
			. '<b>Request Inputs:</b> ' . json_encode($request->except('password', 'password_confirmation'), JSON_UNESCAPED_UNICODE);
		...
		$message .=
			...
			. '<b>Trace: </b> '
			. PHP_EOL
			. '<b> => </b> => ' . substr($exception->getTraceAsString(), 0, 1000) . ' ...';
